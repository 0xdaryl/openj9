...
printf("  REG : regParmCursor : %d, gprRegSpillInstructionCount=%d\n", regParmCursor, gprRegSpillInstructionCount); fflush(stdout);
				} else {
					// Parm must be filled from frame and spilled to argList
					gprRegFillInstructionCount++;
					gprRegSpillInstructionCount++;
printf("  MEM : regParmCursor : %d, gprRegFillInstructionCount=%d, gprRegSpillInstructionCount=%d\n", regParmCursor, gprRegFillInstructionCount, gprRegSpillInstructionCount); fflush(stdout);
				}
				break;
			}
...
printf("  REG : regParmCursor : %d, fprRegSpillInstructionCount=%d\n", regParmCusor, fprRegSpillInstructionCount); fflush(stdout);
					...
					fprRegSpillInstructionCount += 1;
printf("  MEM : regParmCursor : %d, fprRegFillInstructionCount=%d, fprRegSpillInstructionCount=%d\n", regParmCursor, fprRegFillInstructionCount, fprRegSpillInstructionCount); fflush(stdout);
				}
...
printf("  MEM : PASS_STRUCT_IN_MEMORY_POINTER_IN_REG : regParmCursor=%d, copyStructInstructionsByteCount=%d\n", regParmCursor, copyStructInstructionsByteCount); fflush(stdout);
						preserve_RDI_RSI = true;
						break;
...
printf("  MEM : PASS_STRUCT_IN_MEMORY_POINTER_ON_STACK : copyStructInstructionsByteCount=%d\n", copyStructInstructionsByteCount); fflush(stdout);
						preserve_RDI_RSI = true;
						break;
...
printf("  REG : PASS_STRUCT_IN_ONE_GPR : regParmCursor=%d, gprRegSpillInstructionCount=%d\n", regParmCursor, gprRegSpillInstructionCount); fflush(stdout);
						break;
		...
		switch (sigArray[i].type) {
			...
			default:
			{
				...
				switch (mechanism) {
					case PASS_STRUCT_IN_MEMORY_POINTER_IN_REG:
					{
printf("XXXXX PASS_STRUCT_IN_MEMORY_POINTER_IN_REG : regParmCursor=%d, frameOffsetCursor=%d, struct.sizeInByte=%d\n", regParmCursor, frameOffsetCursor, sigArray[i].sizeInByte); fflush(stdout);
...
printf("XXXXX final thunkCursor=%p, bytesUsed=%d\n", thunkMem, (I_32)(thunkCursor - thunkMem)); fflush(stdout);

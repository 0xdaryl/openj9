...
J9::Method::returnType()
   {
   return J9ToTRMap[self()->j9returnType()];
   }
...
J9::Method::returnTypeIsUnsigned()
   {
   return J9ToIsUnsignedMap[self()->j9returnType()];
   }
...
J9::Method::directCallOpCode()
   {
   return J9ToTRDirectCallMap[self()->j9returnType()];
   }
...
J9::Method::indirectCallOpCode()
   {
   return J9ToTRIndirectCallMap[self()->j9returnType()];
   }
...
J9::Method::returnOpCode()
   {
   return J9ToTRReturnOpCodeMap[self()->j9returnType()];
   }
...
J9::Method::returnTypeWidth()
   {
   return J9ToSizeMap[self()->j9returnType()];
   }
...
J9::Method::signature(TR_Memory *trMemory, TR_AllocationKind allocKind)
   ...
   if(!_fullSignature)
      {
      char *s = (char *)trMemory->allocateMemory(self()->classNameLength() + self()->nameLength() + self()->signatureLength() + 3, allocKind);
      sprintf(s, "%.*s.%.*s%.*s", self()->classNameLength(), self()->classNameChars(), self()->nameLength(), self()->nameChars(), self()->signatureLength(), self()->signatureChars());
...
J9::Method::isConstructor()
   {
   return self()->nameLength()==6 && !strncmp(self()->nameChars(), "<init>", 6);
   }
...
J9::Method::isFinalInObject()
   {
   return methodIsFinalInObject(self()->nameLength(), (U_8 *) self()->nameChars(), self()->signatureLength(), (U_8 *) self()->signatureChars()) ? true : false;
   }
...
J9::Method::isBigDecimalMethod(J9ROMMethod *romMethod, J9ROMClass *romClass)
   ...
   return TR_J9VMBase::isBigDecimalClass(J9ROMCLASS_CLASSNAME(romClass)) &&
          TR::Method::isBigDecimalNameAndSignature(J9ROMMETHOD_GET_NAME(romClass, romMethod), J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod));
   }
...
J9::Method::isBigDecimalMethod(J9Method *j9Method)
   ...
   */
   ...
   return TR::Method::isBigDecimalMethod(J9_ROM_METHOD_FROM_RAM_METHOD(j9Method), J9_CLASS_FROM_METHOD(j9Method)->romClass);
   }
...
J9::Method::isBigDecimalMethod(J9UTF8 *className, J9UTF8 *name, J9UTF8 *signature)
   {
   return TR_J9VMBase::isBigDecimalClass(className) && TR::Method::isBigDecimalNameAndSignature(name, signature);
   }
...
J9::Method::isBigDecimalMethod(TR::Compilation *comp)
   {
   return TR::Method::isBigDecimalMethod(_className, _name, _signature);
   }
...
J9::Method::isBigDecimalConvertersMethod(J9ROMMethod *romMethod, J9ROMClass *romClass)
   ...
   return TR_J9VMBase::isBigDecimalConvertersClass(J9ROMCLASS_CLASSNAME(romClass)) &&
          TR::Method::isBigDecimalConvertersNameAndSignature(J9ROMMETHOD_GET_NAME(romClass, romMethod), J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod));
   }
...
J9::Method::isBigDecimalConvertersMethod(J9Method *j9Method)
   {
   return TR::Method::isBigDecimalConvertersMethod(J9_ROM_METHOD_FROM_RAM_METHOD(j9Method), J9_CLASS_FROM_METHOD(j9Method)->romClass);
   }
...
J9::Method::isBigDecimalConvertersMethod(J9UTF8 *className, J9UTF8 *name, J9UTF8 *signature)
   {
   return TR_J9VMBase::isBigDecimalConvertersClass(className) && TR::Method::isBigDecimalConvertersNameAndSignature(name, signature);
   }
...
J9::Method::isBigDecimalConvertersMethod(TR::Compilation *comp)
   {
   return TR::Method::isBigDecimalConvertersMethod(_className, _name, _signature);
   }
...
J9::Method::isUnsafeCAS(TR::Compilation *c)
   {
   TR::RecognizedMethod rm = self()->getRecognizedMethod();
   switch (rm)
      {
...
J9::Method::isVarHandleAccessMethod(TR::Compilation *comp)
   {
   TR::RecognizedMethod rm = self()->getMandatoryRecognizedMethod();
   switch (rm)
      {

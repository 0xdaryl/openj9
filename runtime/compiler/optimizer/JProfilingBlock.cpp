...
class EdgeFrequencyInfo
   ...
   void addAbsoluteEdge(TR::CFGEdge *edge, TR::Block *block)
      ...
      if (trace)
         ...
         printEdge(comp->getLogger(), toAdd, toSub);
         }
      }
...
void TR_JProfilingBlock::addRecompilationTests(TR_BlockFrequencyInfo *blockFrequencyInfo)
  ...
  // add invocation check to the top of the method
   ...
   if (root != NULL)
      ...
      if (trace())
         comp()->dumpMethodTrees(comp()->getLogger(), "Trees after JProfiling");
      }
   else
      {
...
int32_t TR_JProfilingBlock::perform()
   ...
   if (trace())
      comp()->dumpMethodTrees(comp()->getLogger(), "Trees after JProfiling counter insertion");
   ...
   if (trace())
      dumpCounterDependencies(comp()->getLogger(), componentCounters);

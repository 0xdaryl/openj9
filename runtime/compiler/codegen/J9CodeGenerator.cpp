...
J9::CodeGenerator::doInstructionSelection()
   ...
   if (self()->comp()->getOption(TR_TraceCG))
      self()->comp()->getDebug()->setupToDumpTreesAndInstructions(self()->comp()->getLogger(), "Performing Instruction Selection");
...
J9::CodeGenerator::compressedReferenceRematerialization()
   ...
   if (self()->comp()->useCompressedPointers() &&
      ...
      {
      if (self()->comp()->getOption(TR_TraceCG))
         self()->comp()->dumpMethodTrees(self()->comp()->getLogger(), "Trees before this remat phase", self()->comp()->getMethodSymbol());
      ...
      if (self()->comp()->getOption(TR_TraceCG))
         self()->comp()->dumpMethodTrees(self()->comp()->getLogger(), "Trees after this remat phase", self()->comp()->getMethodSymbol());
      ...
      if (self()->shouldYankCompressedRefs())
         ...
         if (self()->comp()->getOption(TR_TraceCG))
            self()->comp()->dumpMethodTrees(self()->comp()->getLogger(), "Trees after this yank phase", self()->comp()->getMethodSymbol());
         }
      }
...
J9::CodeGenerator::allocateLinkageRegisters()
   ...
   if (self()->comp()->getOption(TR_TraceOptDetails) && self()->comp()->getLoggingEnabled())
      ...
      self()->getDebug()->print(self()->comp()->getLogger(), &globalRegsWithRegLoad);
   ...
   for(TR::TreeTop *tt = firstBlock->getFirstRealTreeTop(); tt; tt = tt->getNextTreeTop())
      ...
      if (node->getOpCode().isStoreDirect() && node->getSymbol()->isParm())
         ...
         if (self()->comp()->getOption(TR_TraceOptDetails) && self()->comp()->getLoggingEnabled())
            ...
            self()->getDebug()->print(self()->comp()->getLogger(), &killedParms);
            dumpOptDetails(self()->comp(), "\n");
            }
         }
      }

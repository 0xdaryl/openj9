    ...
    bool isCheckCast, TR::CodeGenerator *cg)
{
    ...
    if (clazz && TR::Compiler->cls.isClassArray(comp, clazz)
        ...
        if (!disableInlineObjectArrayCheck && fej9->isJavaLangObject(componentClass)) {
            ...
            if (isCheckCast) {
                // Unsuccessful checkcast cast jump directly to the OOL helper to
            ...
            int32_t numRegDeps = 1 + // objectReg
                ((objectReg != objectClassReg) ? 1 : 0) + 1 + // objectReg helper arg
                1 + // castClass helper arg
                (resultReg ? 1 : 0) + (scratchReg ? 1 : 0);
         ...
         */
            ...
            //
            logprintf(comp->getOption(TR_TraceCG), comp->log(),
                "Inline instanceof/checkcast/isAssignableFrom for const cast class array: node=%p, isCheckCast=%d, "
                "icall?=%d\n",
                node, isCheckCast, (node->getOpCodeValue() == TR::icall));
            ...
            int32_t numRegDeps = 1 + // objectReg
                ((objectReg != objectClassReg) ? 1 : 0) + 1 + // objectReg helper arg
                1 + // castClass helper arg
                (resultReg ? 1 : 0) + (scratchReg ? 1 : 0) + (scratchReg2 ? 1 : 0) + (scratchReg3 ? 1 : 0);

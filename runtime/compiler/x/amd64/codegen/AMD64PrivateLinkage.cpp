...
void J9::X86::AMD64::PrivateLinkage::buildIPIC(TR::X86CallSite &site, TR::LabelSymbol *entryLabel, TR::LabelSymbol *doneLabel, uint8_t *thunk)
   ...
   if (comp()->getGenerateReadOnlyCode())
      {
      buildNoCodePatchingIPIC(site, entryLabel, doneLabel, thunk);
      return;
      }
...
void J9::X86::AMD64::PrivateLinkage::buildNoCodePatchingIPIC(TR::X86CallSite &site, TR::LabelSymbol *entryLabel, TR::LabelSymbol *doneLabel, uint8_t *thunk)
   {
   ...
   TR::SymbolReference *populateMethodInIPICDataSlotSymRef =
      cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64populateMethodInIPICDataSlot, false, false, false);
   ccInterfaceDataAddress->slot1Method = reinterpret_cast<intptr_t>(populateMethodInIPICDataSlotSymRef->getMethodAddress());

   ccInterfaceDataAddress->slot2Class = static_cast<intptr_t>(-1);
   ccInterfaceDataAddress->slot2Method = reinterpret_cast<intptr_t>(populateMethodInIPICDataSlotSymRef->getMethodAddress());

   TR::SymbolReference *IPICResolveNoPatchingSymRef =
      cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64IPICResolveNoPatching, false, false, false);
   ccInterfaceDataAddress->slowInterfaceDispatchMethod = reinterpret_cast<intptr_t>(IPICResolveNoPatchingSymRef->getMethodAddress());
   ...
   //    jne short interfaceDispatchSnippet
   ...
   TR::LabelSymbol *interfaceDispatchSnippetLabel = generateLabelSymbol(cg());

   TR::X86InterfaceDispatchSnippet *snippet = new (trHeapMemory()) TR::X86InterfaceDispatchSnippet(
      interfaceDispatchSnippetLabel,
      site.getCallNode(),
   ...
   generateLabelInstruction(JNE4, site.getCallNode(), interfaceDispatchSnippetLabel, cg());
...
J9::X86::AMD64::PrivateLinkage::buildNoPatchingVirtualDispatchWithResolve(
   ...
   //    jz doResolveVirtualDispatch
   //
   TR::LabelSymbol *resolveVirtualDispatchSnippetLabel = generateLabelSymbol(cg());

   TR::X86ResolveVirtualDispatchSnippet *snippet = new (trHeapMemory()) TR::X86ResolveVirtualDispatchSnippet(
      resolveVirtualDispatchSnippetLabel,
      site.getCallNode(),
   ...
   generateLabelInstruction(JE4, site.getCallNode(), resolveVirtualDispatchSnippetLabel, cg());
